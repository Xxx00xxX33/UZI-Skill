...
def test_distressed_negative_roe():
    ...
    assert detect_style(f, {}) == DISTRESSED, (
        "ST 股（负 ROE）应判为 distressed 不是 small_speculative"
    )
...
def test_fund_managers_no_cap_in_wave3():
    ...
    src = (
        ...
        + "\n"
        + (SCRIPTS_DIR / "lib" / "pipeline" / "score_fns.py").read_text(
            encoding="utf-8"
        )
    )
    ...
    assert bad is None, (
        "BUG#R2 regression: wave3 不应调 fetch_fund_holders.main(..., limit=6)"
    )
...
def test_sig_dist_has_skip_key_in_run_real_test():
    ...
    line = src[idx : idx + 200]
    assert '"skip"' in line, (
        "BUG regression: run_real_test 的 sig_dist 必须含 'skip' key"
    )
...
def test_collect_raw_data_has_timeout():
    ...
    assert "as_completed(futures, timeout=" in snippet, (
        "BUG regression: as_completed 必须带 timeout"
    )
    assert ".result(timeout=" in snippet, (
        "BUG regression: future.result() 必须带 timeout"
    )
...
def test_mini_racer_lock_exists():
    ...
    for fetcher in ("fetch_industry", "fetch_capital_flow", "fetch_valuation"):
        assert fetcher in src, (
            f"BUG regression: {fetcher} 应在 _MINI_RACER_FETCHERS 列表里"
        )
...
def test_all_modules_have_future_annotations():
    ...
    for fn in sorted(SCRIPTS_DIR.rglob("*.py")):
        ...
        has_pep604 = bool(
            re.search(r"def\s+\w+\([^)]*\b\w+\s*:\s*\w+\s*\|\s*(None|\w+)", content)
        )
        if not has_pep604:
            continue
...
def test_ddgs_in_requirements():
    req = (SCRIPTS_DIR.parent.parent.parent / "requirements.txt").read_text(
        encoding="utf-8"
    )
    assert "ddgs" in req.lower(), (
        "BUG regression: ddgs 必须列在 requirements.txt（lib/web_search 依赖）"
    )
...
def test_run_py_version_banner_dynamic():
    ...
    assert "_get_version()" in src, (
        "BUG regression: run.py banner 必须用动态 _get_version()"
    )
    assert "v2.2" not in src or "v2.2 ·" not in src, (
        "BUG regression: 不能硬编码 v2.2 banner"
    )
...
def test_xueqiu_browser_opt_in_only():
    ...
    assert "is_login_enabled" in src, (
        "BUG#R5 regression: xueqiu_browser 必须有 opt-in 检查"
    )
...
def test_fetch_financials_hk_branch_implemented():
    ...
    assert 'elif ti.market == "H":' in src, (
        "BUG#R7 regression: main 未分发 HK 到 _fetch_hk"
    )
...
def test_kline_hk_has_fallback_chain():
    ...
    assert ".HK" in body or "yf.Ticker" in body, (
        "BUG#R8 regression: chain 缺 yfinance 路径"
    )
...
def test_trusted_domains_covers_qualitative_dims():
    ...
    for dim in MUST_HAVE:
        ...
        assert len(domains) >= 3, (
            f"v2.7.3 regression: {dim} 权威域少于 3 个（当前 {len(domains)}）"
        )
...
def test_investor_profile_authentic_per_persona():
    ...
    assert len(PROFILES) >= 22, (
        f"v2.8 regression: authored profiles 少于 22（当前 {len(PROFILES)}）"
    )
    ...
    assert "2 天" in simons["time_horizon"] or "<" in simons["time_horizon"], (
        "simons 必须是超高频"
    )
    # 翻盘条件必须体现不同方法论
    assert "ROE" in buffett["what_would_change_my_mind"], "buffett 翻盘必须与 ROE 有关"
    assert (
        "龙头" in zhao_lg["what_would_change_my_mind"]
        or "板" in zhao_lg["what_would_change_my_mind"]
    )
    assert (
        "Sharpe" in simons["what_would_change_my_mind"]
        or "因子" in simons["what_would_change_my_mind"]
    )
...
def test_investor_profile_group_fallback():
    ...
    assert (
        "均线" in r_d["what_would_change_my_mind"]
        or "趋势" in r_d["what_would_change_my_mind"]
    )
    # 'sunan' 是 group F 游资
    r_f = get_profile("sunan", "F")
    assert (
        "板" in r_f["what_would_change_my_mind"]
        or "龙虎榜" in r_f["what_would_change_my_mind"]
    )
...
def test_quotes_knowledge_base_covers_authored_personas():
    ...
    assert not missing, (
        f"v2.8.1 regression: quotes-knowledge-base 缺以下 authored 人物的原话段落: {missing}"
    )
...
def test_metals_industries_have_materials_coverage():
    ...
    for ind in ("工业金属", "有色金属", "贵金属", "能源金属"):
        assert ind in INDUSTRY_MATERIALS, (
            f"BUG#R10-coverage: INDUSTRY_MATERIALS 缺 {ind!r}"
        )
...
def test_self_review_engine_exists():
    ...
    assert hasattr(self_review, "review_all"), (
        "v2.9 regression: lib.self_review.review_all missing"
    )
    assert hasattr(self_review, "write_review")
    assert hasattr(self_review, "CHECKS")
    assert len(self_review.CHECKS) >= 10, (
        f"v2.9 regression: self-review 检查不足 10 条（当前 {len(self_review.CHECKS)}）"
    )
...
def test_assemble_report_gated_by_review():
    ...
    assert "from lib.self_review" in src, (
        "v2.9 regression: assemble_report 未 import self_review"
    )
    assert "review_all(ticker)" in src, (
        "v2.9 regression: assemble_report 未调 review_all"
    )
...
def test_self_review_catches_bug_r10():
    ...
    ctx = {
        ...
        "dims": {
            ...
            "7_industry": {
                "data": {"cninfo_metrics": {"industry_name_match": "农副食品加工业"}}
            },
        },
    }
    issues = check_industry_mapping_sanity(ctx)
    assert len(issues) >= 1, (
        "BUG#R10 regression: self_review 没抓到工业金属→农副食品加工的误映射"
    )
...
def test_panel_insights_rendered():
    ...
    sc_src = (SCRIPTS_DIR / "lib" / "report" / "special_cards.py").read_text(
        encoding="utf-8"
    )
    merged = ar_src + "\n" + sc_src
    ...
    tpl = (SCRIPTS_DIR.parent / "assets" / "report-template.html").read_text(
        encoding="utf-8"
    )
...
def test_top3_bears_rendered():
    ...
    ar_src = (SCRIPTS_DIR / "assemble_report.py").read_text(encoding="utf-8")
    pc_src = (SCRIPTS_DIR / "lib" / "report" / "panel_cards.py").read_text(
        encoding="utf-8"
    )
    merged = ar_src + "\n" + pc_src
...
def test_consensus_neutral_weighted_formula():
    ...
    assert (
        "NEUTRAL_WEIGHT * neutral" in body
        or "0.6 * neutral" in body
        or "0.5 * neutral" in body
        or "0.5*neutral" in body
...
def test_ticker_parser_sh_etf():
    ...
    assert ti.full == "512400.SH", (
        f"BUG#2.9.2 regression: 512400 应为 SH，实际 {ti.full!r}"
    )
...
def test_stage1_early_exits_on_etf():
    ...
    src = (
        ...
        + "\n"
        + (SCRIPTS_DIR / "lib" / "pipeline" / "preflight_helpers.py").read_text(
            encoding="utf-8"
        )
    )
    assert "non_stock_security" in src, (
    ...
    assert "fund_portfolio_hold_em" in src, "v2.9.2 regression: ETF 持仓拉取接口未使用"
...
def test_fund_holders_two_tier_strategy():
    ...
    lite_end = src.find("\n\n", lite_idx)
    lite_body = (
        src[lite_idx:lite_end] if lite_end > 0 else src[lite_idx : lite_idx + 2000]
    )
...
def test_ddg_budget_enforced():
    ...
    try:
        ...
        results = web_search.search(
            "测试预算 xxxx_unique_probe_q",
            max_results=3,
            cache_key_prefix="budget_test",
        )
...
def test_analysis_profile_three_tiers():
    ...
    # 档位差异必须显著
    assert (
        len(lite.fetchers_enabled)
        < len(mid.fetchers_enabled)
        == len(deep.fetchers_enabled)
    )
...
def test_net_timeout_guard_exists():
    ...
    assert "Session.request" in src, (
        "必须 patch Session.request（覆盖所有 akshare 内部调用）"
    )
...
def test_tushare_provider_requires_key(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        tushare_provider,
        "_AI_TRADER_ENV_PATH",
        tmp_path / "missing-ai-trader.env",
    )
...
def test_parse_ticker_hk_3digit():
    ...
    for code in ("700", "981"):
        r = parse_ticker(code)
        assert r.market == "H", (
            f"v2.10.2 regression: {code} 应识别为 HK，实际 {r.market}"
        )
